...
echo "$OUTPUT2"
if [[ $OUTPUT2 == *"\"status\":\"CLAIMED\""* && $OUTPUT2 == *"\"agent\":\"Coding Agent\""* ]]; then
    echo -e "${GREEN}[PASS] Coding Agent claimed the task.${NC}"
else
...
echo "$OUTPUT4"
if [[ $OUTPUT4 == *"\"status\":\"RUNNING\""* && $OUTPUT4 == *"\"assignee\":\"Coding Agent\""* ]]; then
...
# Test Case 6: BLOCKED transition (description contains 'CRASH')
echo -e "\nTest 6: Verifying BLOCKED transition on CRASH keyword..."
...
if [[ $OUTPUT6 == *"\"status\":\"BLOCKED\""* ]]; then
    echo -e "${GREEN}[PASS] Agent correctly detected CRASH and blocked the task.${NC}"
else
...
EXPECTED_PATTERN="1|Fix Database Index|Optimize query speeds|database|2|Coding Agent|3"
